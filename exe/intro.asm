; Listing generated by Microsoft (R) Optimizing Compiler Version 19.00.24213.1 
	...
	TITLE	C:\Users\tab\Documents\Stuff\Shoshin\src\intro.cpp
	.686P
	.XMM
	include listing.inc
	.model	flat
...
PUBLIC	??_C@_0FNB@KPHNDBI@?$CDversion?5430?6out?5vec4?5FragColor?$DL@ ; `string'
;	COMDAT ??_C@_0FNB@KPHNDBI@?$CDversion?5430?6out?5vec4?5FragColor?$DL@
CONST	SEGMENT
??_C@_0FNB@KPHNDBI@?$CDversion?5430?6out?5vec4?5FragColor?$DL@ DB '#versi'
	DB	'on 430', 0aH, 'out vec4 FragColor;in vec2 px;uniform vec4 fpa'
	DB	'r4[4];int XRES = 1280;int YRES = 720;float EPSILON = 0.01;flo'
	DB	'at sdfSphere(vec3 p, float r){return length(p) - r;}float sdf'
	DB	'Scene(vec3 p){p.x = mod(p.x+1, 2) - 1;p.y = mod(p.y+1, 2) - 1'
	DB	';p.z = mod(p.z + 1, 2) - 1;float d = sdfSphere(p, 0.5);return'
	DB	' d;}vec3 estimateSdfNormal(vec3 p){return normalize(vec3(sdfS'
	DB	'cene(vec3(p.x + EPSILON, p.y, p.z)) - sdfScene(vec3(p.x - EPS'
	DB	'ILON, p.y, p.z)), sdfScene(vec3(p.x, p.y + EPSILON, p.z)) - s'
	DB	'dfScene(vec3(p.x, p.y - EPSILON, p.z)), sdfScene(vec3(p.x, p.'
	DB	'y, p.z + EPSILON)) - sdfScene(vec3(p.x, p.y, p.z - EPSILON)))'
	DB	');}vec3 simpleLambert(vec3 normal, vec3 col){vec3 lightDir = '
	DB	'vec3(1.0, 2.0, 4.0);vec3 lightCol = vec3(1.0, 1.0, 1.0);float'
	DB	' NdotL = max(dot(normal, lightDir), 0);vec3 c = vec3(col * li'
	DB	'ghtCol * NdotL);return c;}void main(){vec2 uv = px * vec2(1.7'
	DB	'777, 1.0);vec3 cameraPos = vec3(1.0 + 4.0*sin(fpar4[0][0]), 2'
	DB	'.0*cos(fpar4[0][0]), -4.0);vec3 target = vec3(0);vec3 w = nor'
	DB	'malize(cameraPos - target);vec3 u = normalize(cross(w, vec3(0'
	DB	'.0, 1.0, 0.0)));vec3 v = cross(u, w);vec3 rayOrigin = cameraP'
	DB	'os;vec3 rayDir = normalize(uv.x * u + uv.y * v + 0.75*w);floa'
	DB	't rayDepth = 0.0;vec3 col = vec3(0.2, 0.2, 0.2);for(int i = 0'
	DB	'; i < 100; i++){float d = sdfScene(rayOrigin + rayDepth * ray'
	DB	'Dir);if(d < EPSILON){col = simpleLambert(estimateSdfNormal(ra'
	DB	'yOrigin + rayDepth * rayDir), vec3(0.2, 0.2, 0.8));break;}ray'
	DB	'Depth += d;if(rayDepth >= 100){col = vec3(0.2, 0.2, 0.2);}}Fr'
	DB	'agColor = vec4(col, 1.0);}', 00H		; `string'
CONST	ENDS
fsh	SEGMENT
?fsh@@3PBDB DD	FLAT:??_C@_0FNB@KPHNDBI@?$CDversion?5430?6out?5vec4?5FragColor?$DL@ ; fsh
fsh	ENDS
...
; File c:\users\tab\documents\stuff\shoshin\src\intro.cpp
